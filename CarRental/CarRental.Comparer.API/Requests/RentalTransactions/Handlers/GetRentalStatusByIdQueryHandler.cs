...
public class GetRentalStatusByIdQueryHandler : IRequestHandler<GetRentalStatusByIdQuery, Result<RentalStatusDto>>
{
    ...
    public async Task<Result<RentalStatusDto>> Handle(GetRentalStatusByIdQuery request, CancellationToken cancellationToken)
    {
        ...
        var rentals = await rentalsRepository.FirstOrDefaultAsync(specification, cancellationToken);

        if (rentals?.Provider?.Name is null)
        {
        ...
        var rentalStatusDto = await carComparisonService.GetRentalStatusByIdAsync(rentals.Provider.Name, request.rentalId, cancellationToken);
